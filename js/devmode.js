...
function runDevMode() {
  ...
  box.innerHTML = `
    ...
    <div class="dev-plan-wrap"><canvas id="devCanvas" width="760" height="460"></canvas></div>
    <button class="ai-btn-primary" style="margin-top:12px;" onclick="openDev3D()">🏘️ Lihat Siteplan 3D</button>
    <div class="takeoff-grid" style="grid-template-columns:repeat(4,1fr); margin-top:14px;">
      <div class="mat-card"><div class="mc-ic">🏠</div><div class="mc-q">${p.units}</div><div class="mc-u">unit</div><div class="mc-n">${p.u.name}</div></div>
      <div class="mat-card"><div class="mc-ic">📐</div><div class="mc-q">${(p.saleable).toLocaleString('id')}</div><div class="mc-u">m²</div><div class="mc-n">Luas kavling jual</div></div>
...
function renderScan(c) {
  c.innerHTML = `
    ...
    <div class="ai-card">
      <div class="ai-card-title">1. Unggah Foto Denah</div>
      <input type="file" id="scanFile" accept="image/*" class="ai-input" onchange="scanLoad(event)">
      <div id="scanInfo" class="ai-sub" style="margin-top:8px;">${underlay.dataUrl?'✅ Underlay aktif.':'Belum ada gambar.'}</div>
    </div>
    <div class="ai-card" id="scanControls" style="${underlay.img?'':'display:none'}">
      <div class="ai-card-title">2. Atur Underlay</div>
      <div class="ai-field"><label>Transparansi: <span id="scanOpVal">${Math.round(underlay.opacity*100)}%</span></label>
        <input type="range" min="10" max="100" value="${underlay.opacity*100}" oninput="underlay.opacity=this.value/100; document.getElementById('scanOpVal').textContent=this.value+'%'; render();"></div>
      <div class="ai-field"><label>Skala: <span id="scanScVal">${underlay.scale.toFixed(2)}×</span></label>
        <input type="range" min="20" max="300" value="${underlay.scale*100}" oninput="underlay.scale=this.value/100; document.getElementById('scanScVal').textContent=(this.value/100).toFixed(2)+'×'; render();"></div>
      <div class="ai-toggle-row">
        <div class="ai-chip ${underlay.visible?'active':''}" id="scanVis" onclick="underlay.visible=!underlay.visible; this.classList.toggle('active',underlay.visible); render();">Tampilkan</div>
        <div class="ai-chip" onclick="scanRemove()">Hapus Underlay</div>
      </div>
    </div>
    <div class="ai-card" id="scanStep3" style="${underlay.img?'':'display:none'}">
